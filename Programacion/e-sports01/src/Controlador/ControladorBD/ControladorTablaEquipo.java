...
public class ControladorTablaEquipo {
    ...
    public List<Equipo> llenarEquiposporID(Integer idequipo) throws Exception {
        List<Equipo> llenarEquipos=new ArrayList<>();
    ...
    public Equipo buscarEquipo(Integer idequipo) throws Exception {
        ...
        if (rs.next()) {
            ...
            equipo.setNombre(rs.getString("nombre"));
            equipo.setFechaFundacion(rs.getDate("fecha_fundacion").toLocalDate());
            Integer id_patrocinador = rs.getInt("id_patrocinador"); // Corregir el nombre de la columna

            // Si necesitas usar ControladorBD cb para buscar el patrocinador, asegúrate de que esté correctamente inicializado
