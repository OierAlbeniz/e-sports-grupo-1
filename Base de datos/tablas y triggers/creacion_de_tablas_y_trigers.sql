...
BEGIN
    SELECT ESTADO INTO v_estado_competicion 
    FROM COMPETICION 
...
FOR INSERT OR UPDATE OF SUELDO ON JUGADOR
COMPOUND TRIGGER
 V_SUELDO_TOTAL_EQUIPO JUGADOR.SUELDO%TYPE;
 E_SUELDOMAXINCORRECTO EXCEPTION;
 NUEVO_ID_EQUIPO JUGADOR.ID_EQUIPO%TYPE;
 NUEVO_SUELDO JUGADOR.SUELDO%TYPE;
 ANTIGUO_SUELDO JUGADOR.SUELDO%TYPE;
BEFORE EACH ROW IS 
BEGIN
    NUEVO_ID_EQUIPO := :NEW.ID_EQUIPO;
    NUEVO_SUELDO := :NEW.SUELDO;
    ANTIGUO_SUELDO := :OLD.SUELDO;
END BEFORE EACH ROW;
AFTER STATEMENT IS
BEGIN 
    SELECT SUM(SUELDO) INTO V_SUELDO_TOTAL_EQUIPO
    FROM JUGADOR
    WHERE ID_EQUIPO = NUEVO_ID_EQUIPO;
    
    IF V_SUELDO_TOTAL_EQUIPO + NVL(NUEVO_SUELDO, 0) - NVL(ANTIGUO_SUELDO, 0) > 200000 THEN
...
CREATE OR REPLACE TRIGGER TR_VALIDAR_SUELDO_MIN
BEFORE INSERT OR UPDATE OF SUELDO ON JUGADOR
FOR EACH ROW
DECLARE
    E_SUELDO_MIN EXCEPTION;
    C_SMI CONSTANT NUMBER(5):= 1134;
BEGIN

    IF :NEW.SUELDO < C_SMI THEN
        RAISE E_SUELDO_MIN;
    END IF;
EXCEPTION
    WHEN E_SUELDO_MIN THEN
    RAISE_APPLICATION_ERROR(-20023,'SUELDO MUY BAJO');
    WHEN OTHERS THEN 
